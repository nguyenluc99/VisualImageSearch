...
class OverAll(EvaluatorBase):
    ...
    def __call__(self, query_result: List, gallery_info: List) -> (float, Dict):
        ...
        for i in range(len(query_result)):
            ...
            # gt: true, false array - true if the lable of ranked_neighbors_idx is equal to the label of query_result, otherwise false
            # psudo_score: the score of ranked_neighbors_idx

            aps.append(average_precision_score(gt, pseudo_score[:len(gt)]))

            # deal with 'gallery as query' test
            # first match image is the query image, then remove it from the ranked list
